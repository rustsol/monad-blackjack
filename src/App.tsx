...
// Fix ethereum provider type for ethers
...
const BlockchainBlackjackGame: React.FC<{ walletAddress: string }> = ({ walletAddress }) => {
  ...
  const hit = async () => {
    ...
    try {
      ...
      console.log('🔍 Current game state before hit:', gameState);
      console.log('🔍 Is game active?', gameState?.isActive);
      console.log('🔍 Is dealer turn?', gameState?.dealerTurn);
  ...
  const stand = async () => {
    ...
    try {
      ...
      console.log('🔍 Current game state before stand:', gameState);
      console.log('🔍 Is game active?', gameState?.isActive);
      console.log('🔍 Is dealer turn?', gameState?.dealerTurn);
          ...
          {gameState?.isActive ? (
            <div>
              ...
              {gameState?.playerCards && calculateHandValue(gameState?.playerCards) > 21 && (
                <div style={{color: '#ef4444', marginTop: '0.5rem'}}>💥 BUSTED! Game Over</div>
              )}
            </div>
            ...
            <div>
              ...
              <div style={{marginTop: '0.5rem', fontSize: '0.9rem', color: '#94a3b8'}}>
                Click "Start Game" for a new round
              </div>
            </div>
          ) : (
        ...
        <div className="game-controls">
          {!gameState?.isActive ? (
            ...
            <div className="action-buttons">
              <button
                onClick={hit}
                disabled={isLoading || gameState.dealerTurn || !gameState.isActive || (gameState.playerCards && calculateHandValue(gameState.playerCards) >= 21)}
