...
contract BlackjackGame {
    ...
    function startGame() external payable noActiveGame(msg.sender) {
        ...
        _dealCard(msg.sender, true);  // Dealer first card (face down)
        ...
        if (_getHandValue(game.playerCards) == 21) {
            if (_getHandValue(game.dealerCards) == 21) {
                ...
                _endGame(msg.sender, 1); // Player blackjack wins
            }
        }
    }
    
    function hit() external gameActive(msg.sender) {
        GameState storage game = currentGames[msg.sender];
    ...
    function stand() external gameActive(msg.sender) {
        GameState storage game = currentGames[msg.sender];
        require(!game.dealerTurn, "Not player's turn");
    ...
    function doubleDown() external payable gameActive(msg.sender) {
        GameState storage game = currentGames[msg.sender];
        require(!game.dealerTurn, "Not player's turn");
        require(game.playerCards.length == 2, "Can only double on first two cards");
        require(msg.value == game.bet, "Must double the original bet");
        
        game.bet += msg.value;
        
        // Deal one card and end player turn
        _dealCard(msg.sender, false);
        emit PlayerAction(msg.sender, game.gameId, "doubleDown");
    ...
    function getHandValue(address player, bool isDealer) external view returns (uint256) {
        if (isDealer) {
            return _getHandValue(currentGames[player].dealerCards);
        } else {
            return _getHandValue(currentGames[player].playerCards);
        }
    }
    
    function canDoubleDown(address player) external view returns (bool) {
        GameState memory game = currentGames[player];
        return game.isActive && !game.dealerTurn && game.playerCards.length == 2;
    }
    
    function forfeitGame() external gameActive(msg.sender) {
        _endGame(msg.sender, 2); // Forfeit counts as dealer win
    }
